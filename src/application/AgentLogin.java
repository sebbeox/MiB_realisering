...
public class AgentLogin extends javax.swing.JFrame  {
    ...
    private void loginBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_loginBtnActionPerformed
        ...
        try {
            int antalAgenter = Integer.parseInt(db.fetchSingle("SELECT COUNT(*) FROM AGENT"));
            for (int i = 0; i <= antalAgenter; i++){
                if (Integer.parseInt(agentID.getText()) == Integer.parseInt(db.fetchSingle("SELECT " + i + " FROM AGENT"))){
                    found = true;
                    break;
                }
            }
            if (!found) {
                JOptionPane.showMessageDialog(null, "Ogiltigt AgentID!");
            }
            if (found && new String(lösenord.getPassword()).equals(db.fetchSingle("SELECT LOSENORD FROM AGENT WHERE AGENT_ID = " + agentID.getText()))){
                
                    new AgentPortal(db, agentID.getText()).setVisible(true);
                    AgentLogin.this.dispose();
            }
            else{
